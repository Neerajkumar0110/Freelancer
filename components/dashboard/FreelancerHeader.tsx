...
export function FreelancerHeader({ onMenuClick }: HeaderProps) {
  const { user } = useAuth();
  const pathname = usePathname();

  const navItems = [
    { label: "Feed", href: "/dashboard/freelancer" },
  ...
  return (
    <header className="sticky top-0 z-50 w-full px-4 py-3">
      <div className="mx-auto max-w-7xl">
        <div className="relative flex h-16 items-center justify-between rounded-2xl border border-white/10 bg-[#020617]/40 px-6 backdrop-blur-2xl transition-all duration-300 hover:border-white/20 shadow-[0_0_30px_-15px_rgba(79,70,229,0.3)]">
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-4">
              ...
              <div className="group cursor-pointer">
                ...
                <p className="text-[10px] font-medium tracking-[0.3em] text-indigo-400/70 uppercase pl-4">
                  Portal v2
                </p>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    {isActive && (
                      <motion.div
                        layoutId="navGlow"
                        className="absolute inset-0 -z-10 rounded-lg bg-indigo-500/10 blur-sm"
                      />
                    )}
                    {isActive && (
                      <motion.span
                        layoutId="navUnderline"
                        className="absolute bottom-0 left-4 right-4 h-0.5 bg-gradient-to-r from-transparent via-indigo-500 to-transparent"
                      />
                    )}
                  </Link>
                );
          ...
          <div className="hidden flex-1 px-8 xl:block">
            <div className="relative group mx-auto max-w-md">
              <div className="absolute -inset-0.5 rounded-xl bg-gradient-to-r from-indigo-500/20 to-purple-500/20 opacity-0 blur transition duration-300 group-focus-within:opacity-100" />
              <div className="relative flex items-center gap-3 rounded-xl bg-[#0F172A]/80 border border-white/5 px-4 py-2.5 transition-all group-focus-within:border-indigo-500/50">
                <Search size={15} className="text-gray-500 group-focus-within:text-indigo-400" />
                <input
                  type="text"
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <div>
                <p className="text-[9px] font-bold uppercase tracking-tighter text-gray-500">Credits</p>
                <p className="text-xs font-black text-white">$4,250.00</p>
              </div>
              ...
              <div className="flex items-center gap-3 pl-2">
                <div className="hidden flex-col items-end sm:flex">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[8px] font-black uppercase tracking-[0.2em] text-emerald-500/80">Online</span>
                  </div>
                </div>
                <div className="rounded-full p-0.5 ring-2 ring-indigo-500/20 transition-all hover:ring-indigo-500/50">
                  <UserMenu />
                </div>
              </div>
            </div>
